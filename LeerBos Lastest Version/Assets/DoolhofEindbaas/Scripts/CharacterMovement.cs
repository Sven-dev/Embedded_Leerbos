...
public class CharacterMovement : MonoBehaviour, I_SmartwallInteractable {
    ...
    IEnumerator _Move(float xdistance, int signum)
    {
        ...
        while (speedChange > 0 && Moving)
        {
            ...
            Character.transform.Translate(Vector2.left * speedChange * signum * Time.deltaTime);
            speedChange -= MoveDrag * Time.deltaTime;
            yield return null;

            Grind.volume = speedChange / MoveSpeed * 10;
        }
    }
    ...
    public bool Clamp()
    {
        ...
        Character.transform.position = new Vector2(
